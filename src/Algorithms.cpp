...
namespace BiDijkstra
{
        ...
        const std::vector<int> &parB)
    {
        ...
        for (int c = meeting; c != -1; c = parF[c])
            path.push_back(c);
        std::reverse(path.begin(), path.end());
        // Backward half: meeting → dst (skip meeting itself)
        for (int c = parB[meeting]; c != -1; c = parB[c])
            path.push_back(c);
        return path;
    }
    ...
    ShortestPathResult shortestPath(const Graph &g, int src, int dst)
    {
        ...
        if (best < INF && meeting != -1)
            res.path = reconstructPath(meeting, parF, parB);
        return res;
    }
    ...
    AlgorithmStats shortestPathStats(const Graph &g, int src, int dst)
    {
        ...
        if (src == dst)
            return {"Bidirectional Dijkstra", 0.0, 0, 1, nowUs() - t0};
                         ...
                         const std::vector<double> &distO)
        {
            for (auto [v, w] : g.neighbours(u))
            {
                ...
                if (distO[v] < INF && dist[u] + w + distO[v] < best)
                {
                    best = dist[u] + w + distO[v];
                    meeting = v;
                }
            }
        };
...
// ════════════════════════════════════════════════════════════
//  BENCHMARK: compare all three algorithms
// ════════════════════════════════════════════════════════════
...
namespace Benchmark
{
    ...
    void compare(const Graph &g, int src, int dst)
    {
        ...
        {
            int saved_astar = s1.nodesExplored - s2.nodesExplored;
            int saved_bidi = s1.nodesExplored - s3.nodesExplored;
            std::cout << "\n";
            std::cout << "  A* explored        " << saved_astar
                      << " fewer nodes than Dijkstra ("
                      << (100 * saved_astar / s1.nodesExplored) << "% reduction)\n";
            std::cout << "  BiDi explored      " << saved_bidi
                      << " fewer nodes than Dijkstra ("
                      << (100 * saved_bidi / s1.nodesExplored) << "% reduction)\n";
            std::cout << "\n";
...
namespace TripPlanner
{
    ...
    RoadTripResult planTrip(const Graph &g, const std::vector<int> &cityIds)
    {
        ...
        if (n == 0)
            throw std::invalid_argument("City list is empty.");
        if (n == 1)
            return {0.0, cityIds};
            ...
            for (int j = 0; j < n; j++)
                if (i != j && dist[i][j] >= INF)
                    throw std::runtime_error(
                        "No road connection between \"" + g.city(cityIds[i]).name + "\" and \"" + g.city(cityIds[j]).name + "\".");
        ...
        for (int idx : tour)
            result.order.push_back(cityIds[idx]);
        return result;
    }
